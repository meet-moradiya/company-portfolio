...
export default function BlogCard({ blog }) {
    ...
    return (
        <Link
            href={`/blog/${blog.slug}`}
            className="mil-more border-t-1 border-neutral-200 pt-18 group/card flex flex-col md:flex-row gap-8"
        >
            ...
            <div className="md:w-[55%] flex flex-col justify-between py-8">
                <div className="mil-up text-xs uppercase font-medium tracking-widest mb-2">
                    <span className="text-amber-500">{blog.category}</span> &nbsp; • &nbsp;{" "}
                    {publishedDate}
                </div>
                <h2 className="mil-up text-2xl font-medium">{blog.title}</h2>
                <p className="mil-up text-neutral-500 font-light line-clamp-3">
