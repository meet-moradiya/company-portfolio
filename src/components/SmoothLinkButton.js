...
export default function SmoothLinkButton({ 
    href, 
    text = "default text", 
    containerClassName = "", 
    textClassName = "", 
    ...
    const { navigateWithTransition, prefetchRoute } = useSmoothNavigation();

    // Prefetch the route on component mount for better performance
    useEffect(() => {
        if (href) {
            prefetchRoute(href);
        }
    ...
    const handleMouseEnter = () => {
        // Prefetch on hover for even better performance
        if (href) {
            prefetchRoute(href);
        }
    };
